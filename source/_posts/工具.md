...
#make && make install
命令格式：http_load -p 并发访问进程数 -s 访问时间 需要访问的URL件
...
准备URL件：urllist.txt，件格式是每行一个URL，URL最好超过50－100个测试效果比较好.件格式
如下：
件就是一个本，每行都是一个url，它会从里面随机访问的。
example.url内容:
http://www.licess.cn
http://www.vpser.net
http://soft.vpser.net
...
-c是并发量，-r是重复次数。 urlUsage: ./ab [options] [ http://]hostname[:port]/path
Options are:
...
这个表示同时处理1000个请求并运行100次index.php参数了解了，我们来看运行一条命令来看看它的返回结果
...
测试的结果主要也是看这两个值。当然仅有这两个指标并不能完成对性能的分7，我们还需要对服务器的
cpu、men进行分7，才能得出结论
二、webbench
